...
for cnt in range(0, 1944):
    ...
    filtered_corpus = remove_stopwords(documents_clean[0])

    filtered_corpus = sorted(filtered_corpus.split(" "))

    corpus.append(filtered_corpus)

    filtered_corpus = set(filtered_corpus)
...
for i in range(len(corpus)):
    no_of_keywords_local = len(corpus[i])
    for j in range(len(keywords)):
        cnt = (corpus[i].count(keywords[j]))
        if cnt == 0:
            continue
        tf_local = []
        tf_local.append(i)
...
#   Test query - "Sum of two numbers Magnus tree Given Number diagram integers array string graph"